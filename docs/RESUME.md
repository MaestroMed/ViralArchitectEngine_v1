...
- ✅ **Sous-titres karaoké burn-in OK** : ffmpeg Homebrew est sans libass → j'ai posé un build static **libass** dans `~/FORGE_LIBRARY/bin/ffmpeg` (x86_64 via Rosetta) et pointé `FORGE_FFMPEG_PATH` dessus (dans `.env` + `pipeline_builder` utilise désormais `settings.FFMPEG_PATH`). Les 12 clips sont re-rendus avec karaoké word-by-word (mot actif en jaune, lower-third) — vérifié visuellement. Fix lié : le décalage clip-relatif ne shiftait pas les timestamps des **mots** → captions hors fenêtre (corrigé).
- ⚠️ **Reste pour la qualité** : (a) **cold-open cassé** quand segment > cap 60s (hook hors fenêtre → crash concat FFmpeg) — désactivé pour ce batch ; (b) segments longs (60-205s) trimés au premier 60s — affiner `window_sizes` courts pour des clips serrés sur le punch ; (c) titres auto bruts (LLM Ollama off) — relancer avec Ollama pour de vraies accroches.
