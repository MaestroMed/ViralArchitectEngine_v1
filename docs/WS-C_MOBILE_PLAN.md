...
- **C2 — Realtime WS spine** (layer live progress over C3's polling): WS client w/ reconnect, the 9 message types (throttle TRANSCRIPT_CHUNK/ANALYSIS_PROGRESS), live job overlays, one-tap cancel + recover/pipeline-stop behind confirm.
  - ✅ **C2 DELIVERED 2026-06-17:** `ForgeSocket` (URLSessionWebSocketTask, @MainActor) connects to the **global `/v1/ws`** — which broadcasts only `JOB_UPDATE`/`PROJECT_UPDATE` (no project-channel transcript firehose, so no throttle needed). Reconnect w/ capped backoff + generation-token invalidation + single read/ping loop + app-level ping keep-alive. Wired into Pilote: **live job-progress overlay on project cards** (bar + stage + %), live "N en cours" header chip → **Jobs sheet** (active-first, progress, **cancel** via `POST /v1/jobs/{id}/cancel` behind a confirm dialog), and `PROJECT_UPDATE`-driven status override. Verified: clean build, 22 unit + 8 UI (incl. live Jobs sheet), screenshots (live 62% overlay + Jobs sheet). Multi-agent review: 4 real concurrency bugs fixed (post-stop reconnect race, double readLoop, reconnect-task leak, stale-socket ping).
  - ⚠️ **Backend security note:** the `/v1/ws*` routes have **no auth dependency** (unauthenticated even under LAN auth). Low sensitivity (job/project progress only) but worth adding a token check, esp. over the tunnel.
- **C5 — Stats** (read-only KPIs + top-clips).
  - ✅ **C5 DELIVERED 2026-06-17** (commits: backend `125aeb6`+`563d9a2`, iOS `563d9a2`). First, **fixed the analytics 500** (root cause: the endpoints called `get_overview`/`get_dashboard`/`get_top_clips`/`get_trends`/… which `AnalyticsService` never implemented → AttributeError). Implemented them DB-backed (clip-queue counts, avg/top viral score, top-clips by score, production-per-day trend; external view metrics 0 until a publisher records them; record/update are safe no-ops). Verified live (engine restarted): 4 GET routes 200, 136 clips / avg 67.3. Then the **Stats tab** (5th tab — Accueil/Pilote/Sources/Clips/Stats): KPI cards + Swift Charts production trend + top-clips (tappable → detail). 26 unit + 9 UI tests; review follow-ups applied (days windowing + project_id scoping + projectId/segmentId on top-clips).
  - ⚠️ analytics endpoints are **lifetime/global-by-design** on totals; the range picker scopes the trend + ranking. External view/engagement = 0 until a real publisher (YouTube/TikTok) records performance.
